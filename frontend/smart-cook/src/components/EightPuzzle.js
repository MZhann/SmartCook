...
const EightPuzzle = () => {
    ...
    const renderTile = (value, index) => {
        return (<>
            {isClient ? (<div
                key={index}
                className={`tile ${value === 0 ? "bg-black text-gray-500" : "border-4 border-black hover:bg-opacity-25 text-[#AAE06E]"} w-[118px] h-[118px] text-[96px] flex items-center justify-center cursor-pointer`}
    ...
    return (
        <div className={`w-[500px] flex-col p-4 mt-14 flex justify-around bg-white rounded-3xl`}>
            <div className={'flex justify-center w-full relative'}>
                {!isWin && <h1 className={`flex justify-center text-xl mb-3`}>8 Puzzle Game</h1>}
            ...
            {!isWin ?
                <div className="flex flex-col items-center">
                    ...
                    <button className="text-white bg-[#AAE06E] w-[136px] h-10 rounded-3xl mt-10" onClick={resetPuzzle}>
                        Reset
                    </button>
                </div> :
