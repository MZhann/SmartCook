    ...
    const handleFavClick = async () => {
        try {
            ...
            const response = await axios.post(
                `https://web-production-ad96.up.railway.app/api/v1/recipes/${recipe.id}/add_to_favorites/`,
                recipe.id,
                config
            );
        ...
        <div className="w-[280px] h-[340px] bg-white rounded-lg my-5 ">
            <div className="w-full h-[210px] relative">
                {recipe.world_cuisine && recipe.dish_type && (
                    <div
                        className={
                            "top-[12px] text-[8px] text-center items-center left-[12px] flex absolute flex-row gap-[8px]"
                        }
                    >
                        <div
                            className={
                                "w-[60px] h-[16px] bg-[#DAE8FF] rounded-3xl pt-[3px] text-[#203878]"
                            }
                        >
                        ...
                        <div
                            className={
                                "w-[60px] h-[16px] bg-[#FFE3F3] rounded-3xl pt-[3px] text-[#872D51]"
                            }
                        >
                            {recipe?.dish_type}
                        </div>
                ...
                <div
                    className="absolute right-2 top-2 z-0 hover:cursor-pointer"
                ...
                >
                    <svg
                        className={`hover:cursor-pointer ${
                            !favButtonClicked ? "fill-none" : "fill-[#4c9fed]"
                        }`}
                        width="35"
                        height="35"
                        viewBox="0 0 35 35"
                    ...
                    >
                        <rect
                            ...
                            height="35"
                            rx="17.5"
                            fill={!favButtonClicked ? "white" : "#4c9fed"}
                            fillOpacity="0.8"
                        />
                        <path
                ...
                >
                    {recipe && recipe.image ? (
                        <Image
                            ...
                            height={211}
                            className={
                                "w-[px] z-0 h-[211px] rounded-tl-[8px] rounded-tr-[8px]"
                            }
                            src={recipe.image}
                            alt="Food"
                        />
                        ...
                        <Image
                            className="w-[280xp] h-[211px] object-cover"
                            src={defaultFood}
                            alt="Food"
                        />
                    )}
                        ...
                        <>
                            {recipe.user.photo ? (
                                <Image
                                    ...
                                    height={40}
                                    width={40}
                                    className="mr-2 object-contain rounded-5xl"
                                    alt="profile avatar"
                                />
                            ) : (
                                <Image
