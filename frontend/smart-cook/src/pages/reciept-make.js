...
const RecipeMake = () => {
        ...
        const handleStepImageChange = (index, event) => {
            const updatedSteps = [...steps];
            updatedSteps[index].image = event.target.files[0]; // Directly set the file object as image
        ...
        const handlePublish = async () => {
            ...
            mainImage && formData.append('image', mainImage); // assuming mainImage is a File object
            ...
            axios.post(
                ...
                setIsLoading(false);
                window.location.replace("/all-receipts");
            }).catch((error) => {
